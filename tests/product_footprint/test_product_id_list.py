...
def test_product_id_list_append_valid_product_id():
    product_ids = [ProductId("urn:pathfinder:product:customcode:buyer-assigned:acme-product")]
    product_id_list = ProductIdList(product_ids)
    new_product_id = ProductId("urn:pathfinder:product:customcode:buyer-assigned:acme-product2")
...
def test_product_id_list_insert_valid_product_id():
    product_ids = [ProductId("urn:pathfinder:product:customcode:buyer-assigned:acme-product")]
    product_id_list = ProductIdList(product_ids)
    new_product_id = ProductId("urn:pathfinder:product:customcode:buyer-assigned:acme-product2")
    product_id_list.insert(0, new_product_id)
    assert len(product_id_list) == 2
    assert product_id_list.product_ids[0] == new_product_id
...
def test_product_id_list_remove_invalid_product_id():
    ...
    with pytest.raises(ValueError, match="product_id is not in the list"):
        product_id_list.remove(ProductId("urn:pathfinder:product:customcode:buyer-assigned:acme-product2"))